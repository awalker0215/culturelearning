...
<body class="mybody">
					...
					<tr>
						<td align="center" valign="center">
							...
							<div class="[ form-group ]">
								<input type="checkbox" value="1" onClick="culfunction()"
									name="class_1" id="fancy-checkbox-activity" autocomplete="off" />
								<div class="[ btn-group ]">
									<label for="fancy-checkbox-activity"
										class="[ btn btn-primary ]"> <span
										class="[ glyphicon glyphicon-ok ]"></span> <span> </span>
									</label> <label for="fancy-checkbox-activity"
										class="[ btn btn-default active ]"> 活動 </label>
								</div>
						...
						<td align="center" valign="center">
							<div class="[ form-group ]">
								<input type="checkbox" value="2" onClick="bulfunction()"
									name="class_2" id="fancy-checkbox-build" autocomplete="off" />
								<div class="[ btn-group ]">
									<label for="fancy-checkbox-build" class="[ btn btn-primary ]">
										<span class="[ glyphicon glyphicon-ok ]"></span> <span> </span>
									</label> <label for="fancy-checkbox-build"
										class="[ btn btn-default active ]"> 建築 </label>
								</div>
							</div>
						</td>
						<td align="center" valign="center">
							<div class="[ form-group ]">
								<input type="checkbox" value="3" onClick="anifunction()"
									name="class_3" id="fancy-checkbox-animal" autocomplete="off" />
								<div class="[ btn-group ]">
									<label for="fancy-checkbox-animal" class="[ btn btn-primary ]">
										<span class="[ glyphicon glyphicon-ok ]"></span> <span> </span>
									</label> <label for="fancy-checkbox-animal"
										class="[ btn btn-default active ]"> 生物 </label>
								</div>
							</div>
						</td>
					</tr>
	...
	<script type="text/javascript">
			...
			function bindInfoWindow(marker, map, infoWindow, html) {
		        ...
		        function culfunction()
				{
	    			var num = 0;
	   				if(document.all("class_1").checked)
	    			{
	   					//alert(1)
	        			num=1
	        			//document.getElementById("class_2").checked = false;
	        			//document.getElementById("class_3").checked = false;
	        			cleanall()
        				var infoWindow_marker = new google.maps.InfoWindow();
						<c:forEach items="${allcultureinfo}" var="q">
		 					latLng = new google.maps.LatLng("${q.l_xaxis}", "${q.l_yaxis}");
		  					marker = new google.maps.Marker
		  					({
		            			position: latLng,
		            			map:map
		  					...
		  					markers.push(marker);				  
		  					var html = "<a href="+"${address}"+"Reception?id="+"${q.l_id}"+">"+"<p>"+"名稱：" + "${q.l_name}"+"</p>"+"</a>";
          					var dital = "${q.l_id}";
          					bindInfoWindow(marker, map, infoWindow_marker, html); 
						</c:forEach>
	    			}	
				}
		        function bulfunction()
				{
	    			var num = 0;
	   				if(document.all("class_2").checked)
	    			{
	        			num=1
	        			//document.getElementById("class_1").checked = false;
	        			//document.getElementById("class_3").checked = false;
	        			cleanall()
        				var infoWindow_marker = new google.maps.InfoWindow();
						<c:forEach items="${allbuildinfo}" var="q">
		 					latLng = new google.maps.LatLng("${q.l_xaxis}", "${q.l_yaxis}");
		  					marker = new google.maps.Marker
		  					({
		            			position: latLng,
				...
				{
	    			var num = 0;
	   				if(document.all("class_3").checked)
	    			{
	        			num=1
	        			//document.getElementById("class_2").checked = false;
	        			//document.getElementById("class_1").checked = false;
	        			cleanall()
        				var infoWindow_marker = new google.maps.InfoWindow();
						<c:forEach items="${allanimalsinfo}" var="q">
		 					latLng = new google.maps.LatLng("${q.l_xaxis}", "${q.l_yaxis}");
		  					marker = new google.maps.Marker
		  					({
		            			position: latLng,
